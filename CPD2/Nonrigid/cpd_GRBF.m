...
%while (iter<max_it)  && (sigma2 > 1e-8) %(sigma2 > 1e-8)
    ...
    %!!!!!!!!!!!!!!!!!!!!!!!! Change displacement function to direct warping
    %W=(dP*G+lambda*sigma2*eye(M))\(PX);
...
if viz, disp(['CPD registration succesfully completed: iter ', num2str(iter),', ntol: ', num2str(ntol), ', sigma2: ',num2str(sigma2)] ); end;
