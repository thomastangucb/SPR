%GLTP_REGISTER Rigid, affine, non-rigid point set registration. 
...
function [Transform, C] = GLTP_register(X, Y, opt)


[M,D]=size(Y); [N, D2]=size(X);
% Check the input options and set the defaults
if nargin<2, error('GLTP_register.m error! Not enough input parameters.'); end;
...
if opt.viz, disp(['%%%%% Starting GLTP-' upper(opt.method) ' registration. %%%' ]); tic; end;
...
switch lower(opt.method)
    case 'nonrigid'
        ...
        [C, W, sigma2, iter, T] =cpd_LLE(X, Y, opt.beta, opt.lambda, opt.max_it, opt.tol, opt.viz, opt.outliers, opt.fgt, opt.corresp, opt.sigma2, opt.knn, opt.tau, opt.tau_annealing_handle,opt.lambda_annealing_handle);   
    case 'nonrigid_lowrank'
        [C, W, sigma2, iter, T] =cpd_LLE_lowrank(X, Y, opt.beta, opt.lambda, opt.max_it, opt.tol, opt.viz, opt.outliers, opt.fgt, opt.numeig, opt.eigfgt, opt.corresp, opt.sigma2,  opt.knn, opt.tau);
    otherwise
