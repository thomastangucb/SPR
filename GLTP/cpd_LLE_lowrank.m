%CPD_LLE_lowrank The nonrigid SPR point-set registration with lowrank
...
function  [C, W, sigma2, iter, T] =cpd_LLE_lowrank(X, Y, beta, lambda, max_it, tol, viz, outliers, fgt, numeig, eigfgt, corresp, sigma2, knn, tau)
...
while (iter<max_it) && (ntol > tol) && (sigma2 > 1e-8)
    ...
    ntol=abs((L-L_old)/L);
    disp([' GLTP nonrigid ' st ' (lowrank)  : dL= ' num2str(ntol) ', iter= ' num2str(iter) ' sigma2= ' num2str(sigma2)]);
...
disp('GLTP registration succesfully completed.');
