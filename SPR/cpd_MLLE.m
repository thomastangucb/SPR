%CPD_MLLE The non-rigid CPD point-set registration. It is recommended to use
...
function  [C, W, sigma2, iter, T] =cpd_MLLE(X, Y, beta, lambda0, max_it, tol, viz, outliers, fgt, corresp, sigma2, knn, tau0, tau_annealing_handle, lambda_annealing_handle)
...
while (iter<max_it) && (ntol > tol) && (sigma2 > 1e-8) 
    ...
    if viz
        disp([' SPR nonrigid ' st ' : dL= ' num2str(ntol) ', iter= ' num2str(iter) ' sigma2= ' num2str(sigma2)]);
    end
...
if viz,disp(['SPR registration succesfully completed: iter ', num2str(iter),', ntol: ', num2str(ntol), ', sigma2: ',num2str(sigma2)] );end;
