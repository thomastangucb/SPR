%CPD_MLLE_lowrank The nonrigid SPR point-set registration with lowrank
...
function  [C, W, sigma2, iter, T] =cpd_MLLE_lowrank(X, Y, beta, lambda, max_it, tol, viz, outliers, fgt, numeig, eigfgt, corresp, sigma2, knn, tau)
...
while (iter<max_it) && (ntol > tol) && (sigma2 > 1e-8)
    ...
    L = L + lambda/2*trace(QtW'*S*QtW) + tau/2*trace(T'*Phi*T);
    ntol=abs((L-L_old)/L);
    disp([' SPR nonrigid ' st ' (lowrank)  : dL= ' num2str(ntol) ', iter= ' num2str(iter) ' sigma2= ' num2str(sigma2)]);

    % M-step. Solve linear system for W.
    dP1 = spdiags(P1,0,M,M); % precompute diag(P)
    dP1_Phi = dP1 + tau*sigma2*Phi;
    dP1_Phi_Q = dP1_Phi*Q;
    F = PX - dP1_Phi*Y;

    W=1/(lambda*sigma2)*(F-dP1_Phi_Q*  ( (lambda*sigma2*invS+Q'*dP1_Phi_Q) \ (Q'*F) ) );
...
disp('SPR registration succesfully completed.');
